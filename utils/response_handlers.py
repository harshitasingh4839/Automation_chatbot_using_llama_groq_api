    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": create_extraction_prompt(prompt)}
            ],
            temperature=0.1,
        ...
        if client_name and confidence_score >= 70:
...
def handle_task_management(prompt: str) -> Dict:
    """Handle task-related queries and management"""
    try:
        client = Groq(api_key=os.getenv("GROQ_API_KEY"))
        completion = client.chat.completions.create(
            model="llama3-groq-70b-8192-tool-use-preview",
            messages=[
                {
                    "role": "system",
                    "content": "Extract task information in JSON format with fields: task, priority, deadline"
                },
        ...
        task_details = json.loads(completion.choices[0].message.content)
        return {
            "response_type": "task",
...
def format_response(response_data: Dict) -> str:
    ...
    elif response_data["response_type"] == "task":
        details = response_data["details"]
        response = f"Task: {details.get('task', 'Unnamed task')}"
        if details.get("priority"): response += f"\nPriority: {details['priority']}"
        if details.get("deadline"): response += f"\nDeadline: {details['deadline']}"
        return response
...
def determine_intent(prompt: str) -> str:
    ...
    if any(word in prompt_lower for word in ["task", "todo", "reminder", "deadline"]):
        return "task"
